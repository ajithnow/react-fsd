...
export const authHandler = [
  // Login
  http.post<never, ApiResponse<AuthResponse> | ApiResponse<null>>('/api/auth/login', async ({ request }) => {
    let body: LoginRequest;
    try {
      body = await request.json() as unknown as LoginRequest;
    } catch (error) {
      // Handle cases where the request body is not valid JSON or is empty
      const errorMessage = error instanceof Error ? error.message : 'Invalid request body format';
  ...
  // Register
  http.post<never, ApiResponse<AuthResponse> | ApiResponse<null>>('/api/auth/register', async ({ request }) => {
    ...
    // Validate essential fields are present
    if (!body || typeof body.name !== 'string' || typeof body.username !== 'string' || typeof body.password !== 'string') {
      return HttpResponse.json<ApiResponse<null>>(
        {
          data: null,
          message: 'Name, username, and password are required for registration',
          success: false
        },
        { status: 400 }
      );
  ...
  http.get<never, ApiResponse<User> | ApiResponse<null>>('/api/auth/me', ({ request }) => {
  ...
  // Refresh token
  http.post<never, ApiResponse<{ accessToken: string; refreshToken: string }> | ApiResponse<null>>('/api/auth/refresh', async ({ request }) => {
    ...
    return HttpResponse.json<ApiResponse<{ accessToken: string; refreshToken: string }>>({
      data: {
        accessToken,
        refreshToken
      },
  ...
  // Logout
  http.post<never, ApiResponse<null>>('/api/auth/logout', async ({ request }) => {
    const authHeader = request.headers.get('Authorization')
    // Extract token, ensuring it's a string and removing 'Bearer ' prefix
  ...
  http.put<never, ApiResponse<null>>('/api/auth/change-password', async ({ request }) => {
    const authHeader = request.headers.get('Authorization')
    // Extract token, ensuring it's a string and removing 'Bearer ' prefix
    const token = authHeader?.startsWith('Bearer ') ? authHeader.replace('Bearer ', '') : null;
    ...
    try {
      body = await request.json() as unknown as { currentPassword: string; newPassword: string };
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Invalid request body format';
      return HttpResponse.json<ApiResponse<null>>(
        {
